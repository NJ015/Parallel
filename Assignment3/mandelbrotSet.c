...
int main() {
    ...
    for (int k=0; k<N; k++){
      ...
      //main loop
      #pragma omp parallel for schedule(dynamic, 1) private(i, j, c)
      // - for: OpenMP directive to parallelize the loops
      // - schedule(dynamic, 1): Dynamic scheduling having a chunk size = 1 (i.e., each thread gets one row at a time)
      // - private(i, j, c): Each thread will have its own private copies of variables i, j, and complex number c to prevent any race conditions
      for (i = 0; i < HEIGHT; i++) {
          for (j = 0; j < WIDTH; j++) {
